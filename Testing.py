...
ax_cb = divider.append_axes('right', size='5%', pad=0.05)
plt.colorbar(im, cax=ax_cb)
im = ax[0,2].imshow((image_map_HbO2_2-np.min(image_map_HbO2_2))/np.max(image_map_HbO2_2-np.min(image_map_HbO2_2)),vmin=vmin_oxi2,vmax=vmax_oxi2)
ax[0,2].axis('off')
divider = make_axes_locatable(ax[0,2])
ax_cb = divider.append_axes('right', size='5%', pad=0.05)
plt.colorbar(im, cax=ax_cb)
im = ax[1,2].imshow((image_map_Hb_2-np.min(image_map_Hb_2))/np.max(image_map_Hb_2-np.min(image_map_Hb_2)),vmin=vmin_deox2,vmax=vmax_deox2)
ax[1,2].axis('off')
